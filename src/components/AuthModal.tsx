...
import { 
  Lock, Mail, Eye, EyeOff, X, ArrowRight,
  AlertTriangle, CheckCircle2, RefreshCw, Activity, Shield, Key
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { signInWithEmail, signUpWithEmail, signIn, AppUserRole } from '../lib/firebase';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleUserAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(`Welcome, ${userName || userEmail}! Logged in successfully.`);
  ...
  const handleAdminAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage('Admin clearance verified. Access granted.');
      window.dispatchEvent(new Event('storage'));
      if (onSuccess) onSuccess(loggedUser);
      setTimeout(() => {
        onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm overflow-y-auto">
      <motion.div
        ...
        className="bg-[#f8fafc] sm:bg-slate-50/80 rounded-[2.5rem] p-4 sm:p-8 max-w-[460px] w-full relative my-auto shadow-2xl"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-full hover:bg-slate-200/70 text-slate-400 hover:text-slate-700 transition-colors z-10"
          title="Close"
        >
          <X size={20} />
        </button>

        {/* Top Header with Heartbeat Pulse Logo */}
        <div className="flex flex-col items-center text-center pt-2 pb-6">
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1.5 max-w-xs font-normal">
            {isForgotPassword
              ? 'Enter your email to receive recovery instructions'
              : isRegistering
                ? 'Register your clinical credentials to begin scanning'
                : activeTab === 'admin'
                  ? 'Surveillance clearance for epidemiological models and telemetry'
                  : 'Access your skin lesion analysis history and dashboard'}
          </p>
        </div>

        {/* Inner Card Container matching the screenshot design */}
        <div className="bg-white rounded-3xl p-6 sm:p-7 shadow-[0_4px_25px_rgba(0,0,0,0.05)] border border-slate-100/90 space-y-5">
          ...
          {isForgotPassword ? (
            ...
            <>
              {/* Quick Demo Preset pill for user convenience */}
              {!isRegistering && (
                <div className="p-2.5 bg-blue-50/70 border border-blue-100 rounded-2xl flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold bg-[#1864FF] text-white px-2.5 py-1 rounded-lg hover:bg-blue-600 transition-colors shadow-xs flex-shrink-0"
                  >
                    Quick Fill
                  </button>
                </div>
              ...
              <form onSubmit={handleUserAuth} className="space-y-4">
                ...
                {isRegistering && (
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="Clinic Worker">Clinic Worker (ANM / ASHA / Primary Care)</option>
                ...
                {/* Primary Blue Button matching screenshot */}
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-[#1864FF] hover:bg-blue-600 text-white font-bold py-3.5 rounded-xl text-sm transition-all shadow-md shadow-blue-500/25 disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {isLoading ? (
              ...
              {!isRegistering && (
                <>
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold py-3 rounded-xl text-sm transition-all flex items-center justify-center space-x-2.5 shadow-2xs hover:shadow-xs"
                  >
            ...
            <>
              <form onSubmit={handleAdminAuth} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-teal-900 hover:bg-teal-800 text-white font-bold py-3.5 rounded-xl text-sm transition-all shadow-md shadow-teal-900/20 disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {isLoading ? (
                    <RefreshCw size={18} className="animate-spin" />
                  ) : (
